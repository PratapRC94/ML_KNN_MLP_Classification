...
def manhattan_distance(x1, x2):
    ...
    Args:
        ...
        x2: A numpy array of shape (n_features,).
    """
    return(sum(abs(np.array(x1)-np.array(x2))))
...
def one_hot_encoding(y):
    ...
    """
    label_unique = set(y)
    label_to_int=dict((c, i) for i, c in enumerate(label_unique))
    integer_encoded=[label_to_int[lb] for lb in y]
    onehot_encoded = list()
    for value in integer_encoded:
        label = [0 for _ in range(len(label_unique))]
        label[value] = 1
